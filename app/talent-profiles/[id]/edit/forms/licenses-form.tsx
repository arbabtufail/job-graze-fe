...
import { Award, MapPin, Globe, FileText, Calendar, CheckSquare } from 'lucide-react'
...
export function LicensesForm({ data, onUpdate }: LicensesFormProps) {
  const { control, handleSubmit, formState: { errors } } = useForm<LicensesFormData>({
    resolver: zodResolver(licensesSchema),
    defaultValues: {
      licenseType: data.ProfessionalLicense?.LicenseType || "",
      licenseState: data.ProfessionalLicense?.LicenseState || "",
      licenseCountry: data.ProfessionalLicense?.LicenseCountry || "",
      licenseNumber: data.ProfessionalLicense?.LicenseNumber || "",
      licenseEffectiveDate: data.ProfessionalLicense?.LicenseEffectiveDate || "",
      licenseExpirationDate: data.ProfessionalLicense?.LicenseExpirationDate || "",
      NCLEXRN: data.Certifications?.NCLEXRN || "",
      EURN: data.Certifications?.EURN || "",
      languageExam: data.Certifications?.LanguageExam || "",
    },
  })
  ...
  const onSubmit = (formData: LicensesFormData) => {
    onUpdate({
      ProfessionalLicense: {
        ...
        LicenseEffectiveDate: formData.licenseEffectiveDate,
        LicenseExpirationDate: formData.licenseExpirationDate,
      },
      Certifications: {
        NCLEXRN: formData.NCLEXRN,
        EURN: formData.EURN,
        LanguageExam: formData.languageExam,
      },
    })
  }
  ...
  return (
    <motion.form
      ...
      className="space-y-8"
    >
      <FormHeader icon={Award} title="Licenses & Certifications" color="text-orange-600" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-2">
          ...
          />
          {errors.licenseCountry && <p className="text-red-500 text-sm">{errors.licenseCountry.message}</p>}
        </div>
          ...
          <Controller
            ...
            render={({ field }) => (
              <div className="relative">
                ...
                <Input type="date" id="licenseEffectiveDate" className="pl-10" {...field} />
              </div>
            )}
          />
          {errors.licenseEffectiveDate && <p className="text-red-500 text-sm">{errors.licenseEffectiveDate.message}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          <Controller
            ...
            render={({ field }) => (
              <div className="relative">
                <Calendar className="absolute left-3 top-1/2 transform -translate-y-1/2 text-orange-600" />
                <Input type="date" id="licenseExpirationDate" className="pl-10" {...field} />
              </div>
            )}
          />
          {errors.licenseExpirationDate && <p className="text-red-500 text-sm">{errors.licenseExpirationDate.message}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          {errors.NCLEXRN && <p className="text-red-500 text-sm">{errors.NCLEXRN.message}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          {errors.EURN && <p className="text-red-500 text-sm">{errors.EURN.message}</p>}
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          {errors.languageExam && <p className="text-red-500 text-sm">{errors.languageExam.message}</p>}
        </div>
      </div>

      <div className="flex justify-end space-x-4">
        <Button type="button" variant="outline">Cancel</Button>
        <Button type="submit" className="bg-orange-600 text-white">Save Changes</Button>
      </div>
    </motion.form>
  )
}
